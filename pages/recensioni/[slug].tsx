...
export async function getStaticPaths() {
  const items = getAllByCategory("recensioni");
  const paths = items.map((it:any)=>({ params: { slug: it.front.slug }}));
  return { paths, fallback: false };
}

export async function getStaticProps({ params }: any) {
  const { front, content } = getDoc("recensioni", params.slug);
  ...
  return (
    <article className="card">
      ...
      {front.comments !== false && (
        <div className="card-body">
          <DisqusComments identifier={"/recensioni/" + front.slug} title={front.title} />
        </div>
      )}
    </article>
  );
